...
class ShapeNetH5(data.Dataset):
    def __init__(self, path, train=True, npoints=2048, novel_input=True, novel_input_only=False):
        if train:
            self.input_path = os.path.join(path, 'mvp_train_input.h5')
            self.gt_path = os.path.join(path, 'mvp_train_gt_%dpts.h5' % npoints)
        else:
            self.input_path = os.path.join(path, 'mvp_test_input.h5')
            self.gt_path = os.path.join(path, 'mvp_test_gt_%dpts.h5' % npoints)
