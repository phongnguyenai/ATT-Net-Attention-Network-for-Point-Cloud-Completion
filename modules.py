...
class PointNet_SA_Module_KNN_last(nn.Module):
    def __init__(self, npoint, nsample, in_channel, mlp, if_bn=True, group_all=False, use_xyz=True, if_idx=False):
        """
        Args:
            npoint: int, number of points to sample
            ...
            mlp: list of int,
        """
        super(PointNet_SA_Module_KNN_last, self).__init__()
        self.npoint = npoint
        self.nsample = nsample
        self.mlp = mlp
        self.group_all = group_all
        ...
        last_channel_partial = in_channel
        self.mlp_conv_partial = []
        for out_channel in mlp[:-1]:
            self.mlp_conv_partial.append(Conv2d(last_channel_partial, out_channel, if_bn=if_bn))
            last_channel_partial = out_channel
        self.mlp_conv_partial.append(Conv2d(last_channel_partial, mlp[-1], if_bn=False, activation_fn=None))
        self.mlp_conv_partial = nn.Sequential(*self.mlp_conv_partial)

        last_channel_complete = in_channel
        self.mlp_conv_complete = []
        for out_channel in mlp[:-1]:
            self.mlp_conv_complete.append(Conv2d(last_channel_complete, out_channel, if_bn=if_bn))
            last_channel_complete = out_channel
        self.mlp_conv_complete.append(Conv2d(last_channel_complete, mlp[-1], if_bn=False, activation_fn=None))
        self.mlp_conv_complete = nn.Sequential(*self.mlp_conv_complete)

    def forward(self, xyz, points, idx=None, is_partial=True):
        """
        Args:
            xyz: Tensor, (B, 3, N)
            points: Tensor, (B, f, N)
        ...
        if is_partial:
            new_points = self.mlp_conv_partial(new_points)
        else:
            new_points = self.mlp_conv_complete(new_points)
...
class SeedGenerator(nn.Module):
    ...
    def forward(self, feat):
        ...
        return completion.permute(0, 2, 1)
