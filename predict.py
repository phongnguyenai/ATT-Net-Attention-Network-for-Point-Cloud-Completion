...
def predict(data, cate):
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    gt = data[3].pos.reshape(-1, 8192, 3).cuda()
    partial = data[2].pos.reshape(-1, 2048, 3).cuda()
    batch = data[2].pos.reshape(-1, 2048, 3).shape[0]

    save_o3d(partial[0].detach().cpu().squeeze().numpy(), f"{path}/partial.ply")
    save_o3d(gt[0].detach().cpu().squeeze().numpy(), f"{path}/gt.ply")
    
    with torch.no_grad():
        net_input = data[2].cuda()
        net_pred = model(net_input, batch)

        xyz = net_pred[0].detach().cpu().squeeze().numpy()
        pcd = o3d.geometry.PointCloud()
